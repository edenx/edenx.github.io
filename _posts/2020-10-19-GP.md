...
In the sequel, we will have a look at different definitions of Gaussian field/process with a non-rigorous introduction, then fully theoretical setup. In particular, we make connections between generalised Gaussian random fields to RKHS, which will be similar to what we have seen before (by identifying a compact self-adjoint non-negative operator). In fact, under such setup, we are able find a Gaussian random field being a solution to some Elliptical SPDE driven by Gaussian white noise. However, we will delay the discussion to the next post, where the extension from bounded Euclidean domain to compact Riemannian manifold is also presented.
...
Let $\calX$ be some metric space, and $f: \calX \to \R$ be a random function, then $f\sim \mathrm{GP}(m, k)$ is a Gaussian process if for any $\mathbf{x}\in \calX^n$, $\mathbf{f} = f(\mathbf{x}) \sim \calN(\mathbf{\mu}, K\_{\mathbf{xx}})$, where $\mathbf{\mu} = m(\mathbf{x})$ and $K\_{\mathbf{xx}} = k(\mathbf{x}, \mathbf{x})$, i.e. $\mathbf{f}$ has a multivariate Gaussian distribution. In addition, $f$ is completely defined by its mean function and covariance function $k$. WLOG, assume $\mu\equiv 0$ in the following discussion.

In addition to this definition, [1] also gives a natural view via the generalisation of the Bayesian linear regression (BLR) model, by first transforming the data via a projection $\mathbf{\Phi}$ (consisting of a set of $m$ basis functions, which can be seen as a feature map) into a higher dimensional space preceding the construction of the BLR model, where
<div>
$$
  \begin{align*}
...
<h2 class="section-heading">Rigorous definitions of Gaussian Field and Covariance Operator</h2>
