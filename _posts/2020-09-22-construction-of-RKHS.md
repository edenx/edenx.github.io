...
With the nice properties of Reproducing Kernel Hilbert Space (RKHS), many important results in ML are analysed theoratically with linear operators on these spaces. To dig into the details, we shall first look into fundamental properties of kernels and RKHS, and how it can be constructed from various perspectives.

To begin with, we give a concise introduction to the definitions and fundamental properties of reproducing kernels and RKHS, which many readers who know kernel methods are familiar with. The following two articles will be devoted to constructing a valid kernel (and its RKHS) from a Complete Orthogonal System (CONS) of a seperable Hilbert Space and eigen-decomposition of a linear operator in $\mathcal{L}^2$. 

In the article, we use $\calH$ to denote Hilbert space if without specification; and we use 'linear operator' and 'linear map' interchangeably. 

<h2 class="section-heading">Basics and fundamental properties of RKHS</h2>

**Definition 1 (Hilbert Space).**_Let $\calH$ be a vector space with an inner product $\inner{\cdot, \cdot}_{\calH}$, and the assoicated norm $\norm{\cdot}_{\calH}$, where
$$
\norm{f}_{\calH} = \sqrt{\inner{f,f}_{\calH}},\; \forall f\in \calH
$$
Then the vector space $\calH$ is a Hilbert space if it is complete $w.r.t.$ the norm._

Note that a Hilbert space is a infinite dimensional generalisation of Euclidean space, which allows working with functions. The following definition gives a special space of functions with smoothness properties.
...
In the following, we will see the correspondance of $k$ and $\calH$ is one-to-one. But first, one more definition is introduced to characterise the bilinear form $k$.
...
$$
It is said to be strictly positive definitte if $h\equiv 0$, i.e. function $h$ is the zero function.
-

Immediately from the definiton, we can see any reproducing kernel is a positive definite function, or positive semi-definite (PSD) kernel in the ML community. The opposite that any positive definite function induces a RKHS is more important and requires some derivations. 
...
_Proof._ We here give a walk-through of the proof, for full details please refer to [].
...
Here, we only prove for the necessity, see [] for full details. 
_Proof._
...
In fact, this construction of RKHS via linear operator on a Hilbert space can be generalised in a more practically useful way, i.e. when we have a particular feature map or a positive definite function (PSD kernel), as we will see in the following posts. Moreover, this view allows the smoothness property of RKHS to be analysed in spectral domain, which gives connections to Harmonic analysis.


<h2 class="section-heading">References</h2>

1. Saburou Saitoh and Yoshihiro Sawano. Theory of reproducing kernels and applications. Springer, 2016.
2. Zaid Harchaoui, UW STAT 538 Lecture 1 handout. 2019.
